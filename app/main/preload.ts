...
const api = {
  ...
  backupLibrary: (targetPath: string, password: string) => ipcRenderer.invoke(IPC_CHANNELS.BACKUP_LIBRARY, { path: targetPath, password }),
  importLibrary: (sourcePath: string, password = "") => ipcRenderer.invoke(IPC_CHANNELS.IMPORT_LIBRARY, { path: sourcePath, password }),
  createAutoBackup: () => ipcRenderer.invoke(IPC_CHANNELS.CREATE_AUTO_BACKUP),
